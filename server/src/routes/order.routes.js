...
router.post("/", authenticate, authorize("customer"), async (req, res, next) => {
  ...
  try {
    ...
    if (!Array.isArray(items) || items.length === 0 || !delivery) {
      return res.status(400).json({ message: "Order items and delivery details are required" });
    }
    session.startTransaction();

    const ids = items.map(item => item.productId);
    const products = await Product.find({ _id: { $in: ids }, active: true }).session(session);
    ...
    for (const [sellerId, group] of groups) {
      const orderItems = group.map(({ product, quantity }) => ({
        product: product._id, seller: product.seller, name: product.name,
        image: product.images?.[0] || "", quantity, price: product.price
      }));
      const subtotal = orderItems.reduce((sum, item) => sum + item.price * item.quantity, 0);
      const deliveryFee = subtotal >= 999 ? 0 : 49;
      const order = await Order.create([{
        orderNumber: `CV-${Date.now().toString().slice(-7)}-${Math.floor(Math.random() * 90 + 10)}`,
      ...
      for (const { product, quantity } of group) {
      ...
      await Notification.create([{
        ...
        message: `Order ${saved.orderNumber} was placed by a customer.`
      }], { session });
      created.push(saved);
    }
